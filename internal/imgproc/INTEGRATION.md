# Guía de integración — DevForge Image Processor

Esta guía explica cómo usar el motor de imágenes Rust desde el código Go
de este proyecto. Los archivos ya están en su lugar:

```
dev-forge-mcp/
├── bin/
│   └── devforge-imgproc        # binario Rust (motor de procesamiento)
├── internal/
│   └── imgproc/
│       └── imgproc.go          # cliente Go (bridge hacia el binario)
└── ...
```
...
### Binario Rust — `bin/devforge-imgproc`

Este es el motor que hace el procesamiento real. Requisitos:
- Debe tener permisos de ejecución (`chmod +x bin/devforge-imgproc`).
- Si necesitas recompilarlo: `make build-rust` en Linux/macOS.
...
> y cópialo a `bin/devforge-imgproc`.

### Cliente Go — `internal/imgproc/imgproc.go`
...
## 2. Cómo usarlo en el código Go del proyecto

Importa el package desde la raíz del módulo:

```go
import "dev-forge-mcp/internal/imgproc"
```
...
## 3. Integración en el servidor MCP (`cmd/dev-forge-mcp`)

Patón recomendado para el servidor MCP de este proyecto:
...
- [ ] `bin/devforge-imgproc` tiene permisos de ejecución (`chmod +x bin/devforge-imgproc`)
- [ ] El módulo Go declara el path correcto en `go.mod` (`module dev-forge-mcp`)
- [ ] El import usa `"dev-forge-mcp/internal/imgproc"`
- [ ] La ruta al binario apunta a `./bin/devforge-imgproc` (relativa al CWD del proceso)
